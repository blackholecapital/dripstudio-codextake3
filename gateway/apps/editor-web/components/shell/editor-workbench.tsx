...
import { getClientSlug } from '@gateway/env-sdk';
...
const SNAPSHOT_KEY = 'gateway-editor-snapshot-v1';
...
export function EditorWorkbench() {
  const [state, setState] = useState<EditorRuntimeState>(() => createInitialState(getClientSlug({ CLIENT_SLUG: process.env.NEXT_PUBLIC_CLIENT_SLUG })));
  const [error, setError] = useState<string | null>(null);
  const [snapshotStatus, setSnapshotStatus] = useState<string | null>(null);
  const [deployStatus, setDeployStatus] = useState<{ state: 'idle' | 'loading' | 'success' | 'error'; target: 'demo' | 'gateway' | null; message: string | null; liveUrl: string | null }>({
    state: 'idle',
    target: null,
    message: null,
    liveUrl: null
  ...
  const runDeploy = async (target: 'demo' | 'gateway') => {
    setDeployStatus({ state: 'loading', target, message: `Deploying to ${target}...`, liveUrl: null });

    try {
      const response = await fetch(`/api/deploy-${target}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          initiatedBy: state.clientSlug,
          target,
          contract
        })
      });
      ...
      setDeployStatus({
        state: 'success',
        target,
        message: `Deployed to ${result.target.label} (${result.mode}).`,
      ...
      setDeployStatus({
        state: 'error',
        target,
        message: deployError instanceof Error ? deployError.message : 'Deploy failed.',
        liveUrl: null
      });
    }
  };
      ...
      <div className="grid flex-1 gap-4 xl:grid-cols-[260px_minmax(0,1fr)_340px]">
        ...
        <WorkspacePanel
          ...
          onSaveSnapshot={() => {
            ...
            window.localStorage.setItem(SNAPSHOT_KEY, createLocalSnapshot(state));
            setSnapshotStatus('Saved snapshot to local storage.');
          }}
          onLoadSnapshot={() => {
            if (typeof window === 'undefined') {
              return;
            ...
            if (!snapshot) {
              setSnapshotStatus('No local snapshot found.');
              return;
            }

            const loaded = loadLocalSnapshot(snapshot);
            if (!loaded.ok) {
              setSnapshotStatus(loaded.error ?? 'Snapshot load failed.');
              return;
            }

            setState(loaded.state);
            setSnapshotStatus('Loaded snapshot from local storage.');
            setError(null);
          }}
          onDeployDemo={() => runDeploy('demo')}
          onDeployGateway={() => runDeploy('gateway')}
          actions={actions}
        />
