...
export function WorkspacePanel({
  ...
  return (
    <GlassPanel className="relative min-h-[460px] overflow-hidden p-4 md:p-6">
      ...
      <div className="relative flex h-full flex-col gap-5">
        <div className="flex flex-col items-start justify-between gap-4 md:flex-row">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-white md:text-3xl">Segment 4 deploy-ready workflow</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-300">Mobile-friendly shell, local snapshots, and real demo/gateway deploy actions.</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" disabled={!canSave} onClick={actions.save} className="rounded-xl border border-white/15 bg-white/10 px-3 py-2 text-xs text-white disabled:opacity-40">Save card</button>
        ...
        <div className="flex flex-wrap gap-2 border-t border-white/10 pt-3">
          <button type="button" onClick={onSaveSnapshot} className="rounded-xl border border-cyan-300/40 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-100">Save local snapshot</button>
          <button type="button" onClick={onLoadSnapshot} className="rounded-xl border border-cyan-300/40 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-100">Load local snapshot</button>
          <button type="button" disabled={deployStatus.state === 'loading'} onClick={onDeployDemo} className="rounded-xl border border-violet-300/40 bg-violet-400/10 px-3 py-2 text-xs text-violet-100 disabled:opacity-40">Deploy Demo</button>
          <button type="button" disabled={deployStatus.state === 'loading'} onClick={onDeployGateway} className="rounded-xl border border-fuchsia-300/40 bg-fuchsia-400/10 px-3 py-2 text-xs text-fuchsia-100 disabled:opacity-40">Deploy Gateway</button>
        </div>
        ...
        {snapshotStatus ? <p className="rounded-xl border border-cyan-400/30 bg-cyan-400/10 px-3 py-2 text-xs text-cyan-100">{snapshotStatus}</p> : null}
        {deployStatus.message ? (
          <p className={`rounded-xl border px-3 py-2 text-xs ${deployStatus.state === 'error' ? 'border-rose-400/40 bg-rose-500/10 text-rose-100' : deployStatus.state === 'success' ? 'border-emerald-400/40 bg-emerald-500/10 text-emerald-100' : 'border-white/20 bg-white/10 text-slate-200'}`}>
            {deployStatus.message}
            {deployStatus.liveUrl ? <a className="ml-2 underline" href={deployStatus.liveUrl} target="_blank" rel="noreferrer">Open live URL</a> : null}
          </p>
        ) : null}

        <div className="relative min-h-[240px] rounded-[20px] border border-dashed border-white/10 bg-slate-950/35 md:min-h-[260px]">
